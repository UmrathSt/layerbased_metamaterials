...
f = data[:,0] # in GHz
D = 76.93*1e-3 # 0.4 mm fr4 thickness
...
#lz = 4e-3 # 4 mm FR4
Z0 = 376.73
Zslab = 1j*Z0/np.sqrt(epsFR4)*np.tan(2*np.pi*f/3e8*np.sqrt(epsFR4)*lz)


plt.plot(f/1e9, np.real((1+S11)/(1-S11))*Z0, "b-", label="Re(Z) from S, 76.9")
plt.plot(f/1e9, np.imag((1+S11)/(1-S11))*Z0, "r-", label="Im(Z) from S, 76.9")
plt.plot(f/1e9, np.real(Z), "bo", markersize=2,label="Re(Z), 76.83")
plt.plot(f/1e9, np.imag(Z), "ro", markersize=2,label="Im(Z), 76.83")
