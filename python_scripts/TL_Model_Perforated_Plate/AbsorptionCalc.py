...
absorber = np.loadtxt("absorber_2.25.txt", delimiter=",")
fss = np.loadtxt("fss_2.25.txt", delimiter=",")
...
D = 40.25e-3
phase = np.exp(4j*np.pi*f*D/3e8)
Rfss = (fss[:,1] + 1j * fss[:,2]) * phase
Zfss = 377*(1 + Rfss) / (1 - Rfss)
Zmeta = Zfss*Zslab/(Zfss+Zslab)

plt.plot(f/1e9, np.abs(absorber[:,1]+1j*absorber[:,2]), "r-", label="absorber")
plt.plot(f/1e9, np.abs((Zmeta - 377) / (Zmeta + 377)), "b-", label="fss")
plt.xlabel("f [GHz]")
plt.ylabel("|S11|")
plt.legend()
plt.show()
