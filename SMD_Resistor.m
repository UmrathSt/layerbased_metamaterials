...
CSX = InitCSX();
resolution = c0/(f_max*sqrt(substrate_epr))/unit /25; % resolution of lambda/50
mesh.x = SmoothMeshLines( [0 MSL_width/2], resolution, 1.5 ,0 );
mesh.x = SmoothMeshLines( [-MSL_length -mesh.x mesh.x MSL_length], resolution, 1.5 ,0 );
mesh.y = SmoothMeshLines( [0 MSL_width/2], resolution/4 , 1.5 ,0);
...
CSX = SetMaterialProperty( CSX, 'FR4', 'Epsilon', substrate_epr, 'Kappa', 2*pi*5e9*0.088*EPS0);
...
[CSX,port{1}] = AddMSLPort( CSX, 999, 1, 'copper', portstart, portstop, 0, [0 0 -1], 'ExcitePort', true, 'FeedShift', 10*resolution, 'MeasPlaneShift',  MSL_length/3, 'Feed_R', 94.5);
...
RunOpenEMS( Sim_Path, Sim_CSX, ['--engine=multithreaded --numThreads=6']);
