function nf2ff = fast_postprocess_nf2ff(nf2ff_struct_Path, Sim_Path, f, theta, theta_off, phi, phi_off, paramstr,  namestr, Mirror= {0, 'PEC', 0})
    ...
    if ~strcmp(mir, 'None');
        display('Using symmetries for the nf2ff');
        nf2ff = CalcNF2FF(nf2ff, Sim_Path, f, theta+theta_off, phi+phi_off, 'Mode',1, 'Mirror', Mirror);
    else;
        display('Using no symmetries');
        nf2ff = CalcNF2FF(nf2ff, Sim_Path, f, theta+theta_off, phi+phi_off, 'Mode',1);
    end;
       for Fn = 1:numel(f);
    rcs = 4*pi./Pin(Fn).*nf2ff.P_rad{Fn}(1);
