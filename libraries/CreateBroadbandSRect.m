function [CSX, params] = CreateBroadbandSRect(CSX, object, translate, rotate);
...
% modified by splitting the outer rect at the lower right corner
  ...
  params = ['# broadband splitted rect absorber made of ',  material, ' at center position x = ', num2str(ocenter(1)), ' y = ', num2str(ocenter(2)), ' z = ' num2str(ocenter(3)), '\n' ...
            '# edge lengths L1, w1=', num2str(L1,'%.4f') ', ' num2str(w1,'%.4f'), ' inner rect L=', num2str(L2,'%.4f'), ', ', ', gap =' num2str(gap, '%.4f'), ' split_gap = ' num2str(split_gap), 'background material ', bmaterial, '\n'];
  return;
end