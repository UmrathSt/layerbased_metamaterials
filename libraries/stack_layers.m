 ...
 %% development
  ...
  if do_xy_meshing;
     try;
        ...
        try;
          resheigth = object.lz;
          substheight = object.AlO2.height;
          zvals = horzcat(zvals, [zvals(end)-subst.height+resheight/2]);
          zvals = horzcat(zvals, [zvals(end)-resheight/2]);
        catch lasterror;
        end;
    ...
    try;
      ...
      try;
        s = object.s;
        xvals = horzcat(xvals, [-s/sqrt(2), -s/2, s/2,s/sqrt(2)]);
        yvals = horzcat(xvals, [-s/sqrt(2), -s/2, s/2, s/sqrt(2)]);
      catch lasterror;
      end;
      try;
        L3 = object.L3;
  ...
  if not(UC.grounded);
    ...
    mesh.z = SmoothMeshLines([-7*UC.lz/8, zvals, 1*UC.lz/8], UC.dz, 1.2);
  end;
