function [CSX, port] = definePBCPorts(CSX, mesh, f_start, polarization, phi_xy, theta_z);
  physical_constants;
  n_cells_to_edge = 20;
  ...
  k = [sin(theta_z)*cos(phi_xy), sin(theta_z)*sin(phi_xy), cos(theta_z)];
  if strcmp(polarization.name, "TE");
    display("TE POLARIZED PBC Excitation \n");
    E = [cos(polarization.angle), sin(polarization.angle), 0];
    func_E{1} = ["(" num2str(cos(polarization.angle)) ")*(" num2str(k(1)) ")*x"];
    func_E{2} = ["(" num2str(sin(polarization.angle)) ")*(" num2str(k(2)) ")*y"];
    func_E{3} = "0";
    H = cross(k, E);
    func_H{1} = ["(" num2str(H(1)) ")*(" num2str(k(1)) ")*x"];
    func_H{2} = ["(" num2str(H(2)) ")*(" num2str(k(2)) ")*y"];
    func_H{3} = ["(" num2str(H(3)) ")*(" num2str(k(3)) ")*z"];
  elseif strcmp(polarization.name, "TM");
    display("TM POLARIZED PBC Excitation \n");
    H = [cos(polarization.angle), sin(polarization.angle), 0];
    func_H{1} = ["(" num2str(cos(polarization.angle)) ")*(" num2str(k(1)) ")*x"];
    func_H{2} = ["(" num2str(sin(polarization.angle)) ")*(" num2str(k(2)) ")*y"];
    func_H{3} = "0";
    E = cross(H, k);
    func_E{1} = ["(" num2str(H(1)) ")*(" num2str(k(1)) ")*x"];
    func_E{2} = ["(" num2str(H(2)) ")*(" num2str(k(2)) ")*y"];
    func_E{3} = ["(" num2str(H(3)) ")*(" num2str(k(3)) ")*z"];
  endif;
