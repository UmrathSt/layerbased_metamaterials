function stacked_rects(UCDim, name, fr4_thickness, edge_lengths, kappa, epsRe, tand, complemential);
  physical_constants;
  UC.layer_td = 0;
  UC.layer_fd = 0;
  UC.td_dumps = 0;
  ...
  for l = edge_lengths;
    p = polygon;
    s = substrate;
    s.lz = fr4_thickness(idx);
    idx = idx+1;
    p.bmaterial.name = horzcat(s.material.name, horzcat('b ', num2str(l)));
    p.bmaterial.Epsilon = s.material.Epsilon;
    p.bmaterial.Kappa = s.material.Kappa;
    p.prio = p.prio+1;
    p.material.name = horzcat(p.material.name,num2str(l));
    s.material.name = horzcat(s.material.name,num2str(l));
    rect{1} = [[-l/2;-l/2],[-l/2;l/2],[l/2;l/2],[l/2;-l/2]];
    p.pts = rect;
