function single_asy_ring(UCDim, fr4_thickness, R1, w1, N, L, w2, phi0, io, eps_subs, tand, mesh_refinement, complemential);
  physical_constants;
  UC.layer_td = 0;
  UC.layer_fd = 1;
  UC.td_dumps = 0;
  ...
  UC.s11_filename_prefix = ['UCDim_' num2str(UCDim) '_lz_' num2str(fr4_thickness) '_R_' num2str(R1) '_w1_' num2str(w1) '_N_' num2str(N) '_phi0_' num2str(phi0) '_L_' num2str(L) '_w2_' num2str(w2) '_eps_' num2str(eps_subs) '_tand_' num2str(tand) '_pos_' io];
  if complemential;
    UC.s11_filename_prefix = horzcat(UC.s11_filename_prefix, '_comp');
  end;
  UC.s11_filename = 'Sparameters_';
  UC.s11_subfolder = 'single_asy_ring';
  UC.run_simulation = 1;
  UC.show_geometry = 1;
  UC.grounded = 1;
  UC.unit = 1e-3;
  ...
  % circle
  asyring.name = 'asy ring';
  asyring.lz = 0.1;
  asyring.rotate = 0;
  asyring.material.name = 'copperRing';
  asyring.material.Kappa = 56e6;
  asyring.material.type = 'const';
  asyring.barmaterial.name = 'bar';
  asyring.barmaterial.Kappa = 56e6;
  asyring.barmaterial.type = 'const';
  asyring.bmaterial.name = 'air';
  asyring.bmaterial.type = 'const';
  asyring.bmaterial.Epsilon = 1;
  asyring.bmaterial.tand = 0;
  asyring.bmaterial.f0 = 10e9;
  asyring.R1 = R1;
  asyring.io = io;
  asyring.L = L;
  asyring.N = N;
  ...
  asyring.UCly = UCDim;
  asyring.prio = 2;
  asyring.xycenter = [0, 0];
  asyring.complemential = complemential;
  
  layer_list = {@CreateUC, UC;  @CreateRect, rectangle;
                                @CreateRect, substrate;
                                @CreateAsyRing, asyring;
                                 };
  material_list = {substrate.material, rectangle.material, asyring.material, asyring.barmaterial, asyring.bmaterial};
