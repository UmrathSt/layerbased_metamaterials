...
% 2015 IEEE International Microwave and RF Conference (IMaRC) 
  ...
  CSX = AddCylinder(CSX, material, object.prio+1,...
        [0,0,-lz/2], [0,0,lz/2], L2/2, 'Transform', {'Translate', translate,'Rotate_Z', rotate});
  ...
  params = ['# broadband circle absorber made of ',  material, ' at center position x = ', num2str(ocenter(1)), ' y = ', num2str(ocenter(2)), ' z = ' num2str(ocenter(3)), '\n' ...
            '# edge lengths L1, w1=', num2str(L1,'%.4f') ', ' num2str(w1,'%.4f'), ' inner disc radius T=', num2str(L2/2,'%.4f'), ', ', ', gap =' num2str(gap, '%.4f'), ', background material ', bmaterial, '\n'];
  return;
end