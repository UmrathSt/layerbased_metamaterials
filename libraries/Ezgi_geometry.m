function Ezgi_geometry(UCDim, fr4_thickness, R1, w1, R2, w2, eps_FR4, complemential, withSubstrate);
  physical_constants;
  UC.layer_td = 1;
  UC.layer_fd = 1;
  UC.td_dumps = 1;
  ...
  if withSubstrate == 1;
    UC.s11_filename_prefix = ["UCDim_" num2str(UCDim) "_lz_" num2str(fr4_thickness) "_R1_" num2str(R1) "_w1_" num2str(w1) "_R2_" num2str(R2) "_w2_" num2str(w2) "_epsFR4_Lorentz_" num2str(eps_FR4) "_withSubstrate"];
  else;
  ...
  UC.s11_subfolder = "Ezgi_geometry";
  UC.run_simulation = 1;
  UC.show_geometry = 0;
  UC.grounded = 0;
  UC.unit = 1e-3;
...
#  dblring.material.Kappa = 56e6;
  ...
  if withSubstrate == 1;
    ...
    layer_list = {@CreateUC, UC; @CreateDoubleRing, dblring};
    material_list = {dblring.material, dblring.bmaterial};  
  endif;
