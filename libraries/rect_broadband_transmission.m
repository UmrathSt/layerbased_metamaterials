function rect_broadband_transmission(UCDim, L1, w1, L2, gap, mesh_refinement, complemential);
  physical_constants;
  UC.layer_td = 0;
  UC.layer_fd = 1;
  UC.td_dumps = 0;
  ...
  UC.s11_filename_prefix = ['UCDim_' num2str(UCDim) '_L1_' num2str(L1) '_w_' num2str(w1) '_L2_' num2str(L2) '_gap_' num2str(gap)];
  if complemential;
    UC.s11_filename_prefix = horzcat(UC.s11_filename_prefix, '_comp');
  end;
  UC.s11_filename = 'Sparameters_';
  UC.s11_subfolder = 'broadband_rect_transmission';
  UC.run_simulation = 1;
  UC.show_geometry = 1;
  UC.grounded = 0;
  UC.unit = 1e-3;
  ...
  if strcmp(UC.polarization, 'y');
    BC = {'PMC', 'PMC', 'PEC', 'PEC', 'PML_8', 'PML_8'}; % boundary conditions
  elseif strcmp(UC.polarization, 'x');
    BC = {'PEC', 'PEC', 'PMC', 'PMC', 'PML_8', 'PML_8'}; % boundary conditions
  else;
    error(['Polarization must be either x or y but got: ' UC.polarization]);
  end;
  ...
  layer_list = {@CreateUC, UC; @CreateBroadbandRect, rect;
                                 };
  material_list = {rect.material, rect.bmaterial};
  [CSX, mesh, param_str] = stack_layers(layer_list, material_list);
  
  [CSX, port] = definePorts(CSX, mesh, UC.f_start, UC.polarization);
