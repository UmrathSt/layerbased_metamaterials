function U_shapes(UCDim, fr4_thickness, L1, w1, L2, w2, eps_subs, tand, mesh_refinement, complemential);
  physical_constants;
  UC.layer_td = 0;
  UC.layer_fd = 0;
  UC.td_dumps = 0;
  ...
  UC.s11_filename_prefix = ['UCDim_' num2str(UCDim) '_lz_' num2str(fr4_thickness) '_L1_' num2str(L1) '_w1_' num2str(w1) '_L2_' num2str(L2) '_w2_' num2str(w2) '_eps_' num2str(eps_subs) '_tand_' num2str(tand)];
  if complemential;
    UC.s11_filename_prefix = horzcat(UC.s11_filename_prefix, '_comp');
  end;
  UC.s11_filename = 'Sparameters_';
  UC.s11_subfolder = 'U_shapes';
  UC.run_simulation = 1;
  UC.show_geometry = 1;
  UC.grounded = 1;
  UC.unit = 1e-3;
  ...
  UC.s11_delta_f = 10e6;
  UC.EndCriteria = 1e-3;
  sbasename = '/mnt/hgfs/E/openEMS/layerbased_metamaterials/Simulation/';
  rbasename = '~/Arbeit/openEMS/git_layerbased/layerbased_metamaterials/Ergebnisse';
  try;
    if strcmp(uname.nodename, 'Xeon');
        sbasename = '/media/stefan/Daten/openEMS/';
        rbasename = '~/Arbeit/openEMS/layerbased_metamaterials/Ergebnisse';
    end;     
    catch lasterror;
  end_try_catch;
  UC.SimPath = [sbasename UC.s11_subfolder '/' UC.s11_filename_prefix];
  UC.SimCSX = 'geometry.xml';
  UC.ResultPath = [rbasename];
  ...
  % circle
  Ushape.name = 'U-shapes';
  Ushape.lz = 0.05;
  Ushape.rotate = 0;
  Ushape.material.name = 'copper rect';
  Ushape.material.Kappa = 56e6;
  Ushape.material.type = 'const';
  Ushape.bmaterial.name = 'air';
  Ushape.bmaterial.type = 'const';
  Ushape.bmaterial.Epsilon = 1;
  ...
  Ushape.prio = 2;
  Ushape.xycenter = [0, 0];
  Ushape.complemential = complemential;
  
  layer_list = {@CreateUC, UC; @CreateRect, rectangle;
                               @CreateRect, substrate;
                               @CreateU, Ushape;
                                 };
  material_list = {substrate.material, rectangle.material, Ushape.material, Ushape.bmaterial};
