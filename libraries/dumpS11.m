function [port] = dumpS11(port, UC);
  ...
  try;
      s11 = s11.* exp(-1j*UC.unit*(UC.port1_zcoordinate-UC.lastz)*2* freq*2*pi/3e8);
      s21 = s21.* exp(-1j*UC.unit*(UC.port2_lgth)*freq*2*pi/3e8);
  catch lasterror;
  end;
  Zin = 376.73 .* (1+s11) ./ (1-s11);
  U1 = port{2}.uf.tot;
  I1 = port{2}.if.tot;
  ...
  fprintf(outfile, ['# Port position is z = ' num2str(port{1}.zcoordinate) ' m times ' num2str(UC.unit) '\n']);
  fprintf(outfile, ['# closest structure elements have z = ' num2str(UC.lastz) ' m times ' num2str(UC.unit) '\n']);
  fprintf(outfile, ['# distance D used for S11 phase-correction (exp(1j*omega*D/c0) is D = ' num2str((port{1}.zcoordinate-UC.lastz)*UC.unit) ' m.\n']);
