function rect_slab_const(UCDim, slab_thickness, mesh_refinement, complemential);
  physical_constants;
  UC.layer_td = 1;
  UC.layer_fd = 0;
  UC.td_dumps = 1;
  ...
  UC.s11_filename_prefix = ['UCDim_' num2str(UCDim) '_lz_' num2str(slab_thickness) '_const'];
  if complemential;
    UC.s11_filename_prefix = horzcat(UC.s11_filename_prefix, '_comp');
  end;
  UC.s11_filename = 'Sparameters_';
  UC.s11_subfolder = 'lorentz_slab';
  UC.run_simulation = 1;
  UC.show_geometry = 1;
  UC.grounded = 0;
  UC.unit = 1e-3;
  ...
  substrate.material.Kappa = 1.0*EPS0*2*pi*1e10;
...
%  substrate.material.tau = [5e-9];
  ...
  material_list = {substrate.material};
