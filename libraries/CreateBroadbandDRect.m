function [CSX, params] = CreateBroadbandDRect(CSX, object, translate, rotate);
...
% 2015 IEEE International Microwave and RF Conference (IMaRC) 
  ...
  souter1 = [-L2/2, -L2/2, -lz/2];
  souter2 = [+L2/2, -L2/2+w2, lz/2];
  ...
  for rot = (0:3)*pi/2;
    CSX = AddBox(CSX, material, object.prio+1,...
        louter1, louter2, 'Transform', {'Translate', translate,'Rotate_Z', rotate+rot});
    CSX = AddBox(CSX, material, object.prio+1,...
        souter1, souter2, 'Transform', {'Translate', translate,'Rotate_Z', rotate+rot});
  end;
  ...
  % now add the gaps
  gstart1 = [-sqrt(2)*gap/2, (L1-L2)/4, -lz/2];
  gstop1  = [+sqrt(2)*gap/2,-(L1-L2)/4-w1/2, +lz/2];
  gstart2 = [-sqrt(2)*gap/2, (L2-L3)/4+w2, -lz/2];
  gstop2  = [+sqrt(2)*gap/2,-(L2-L3)/4, +lz/2];
  CSX = AddBox(CSX, material, object.prio+2, gstart1, gstop1, ...
        'Transform', {'Rotate_Z', pi/4, 'Translate', translate+[-(L1-w1+L2)/4,(L1-w1+L2)/4,0]}); 
  CSX = AddBox(CSX, material, object.prio+2, gstart2, gstop2, ...
        'Transform', {'Rotate_Z', pi/4, 'Translate', translate+[-gap2_op*(L2-w2+L3)/4, gap2_op*(L2-w2+L3)/4,0]}); 
